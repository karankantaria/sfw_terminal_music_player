...
Returns Rich renderables (Group objects) — the caller owns the Live context
and calls live.update(); nothing is printed directly from this module.
"""
...
def player_renderable(
    ...
    code      = _idle_code() if not track else _playing_code(track, features, waveform_lines, wv_width, liked)
    syntax    = Syntax(code, 'python', theme='monokai', line_numbers=True, start_line=142)
    ...
    return Group(tabs, syntax, status, hint)


# ── Panic renderable ──────────────────────────────────────────────────────────

def panic_renderable(scroll_offset: int, panic_code: str) -> tuple:
    """Return (Group, visible_line_count)."""
    ...
    return Group(tabs, syntax, status, hint), visible
